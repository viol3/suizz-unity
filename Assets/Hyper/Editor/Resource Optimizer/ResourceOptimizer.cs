...
public class ResourceOptimizer : EditorWindow
{
	...
	[MenuItem("Window/Hyper/Resource Optimizer")]
	static void Init()
	{
		ResourceOptimizer window = (ResourceOptimizer)EditorWindow.GetWindow(typeof(ResourceOptimizer));
		window.titleContent = new GUIContent("Resource Optimizer");
	...
	{
		defColor = GUI.color;
        EditorGUILayout.LabelField("Resource Optimizer by Hyper.gs", Hyper.EditorUtility.BoldLabel(TextAnchor.MiddleCenter, 12, Color.white));
        Hyper.EditorUtility.HorizontalLine(2, Color.gray);
        EditorGUILayout.LabelField("Optimize textures and check how many assets you have in the scene.", Hyper.EditorUtility.BoldLabel(TextAnchor.MiddleCenter, 11, Color.white));
        Hyper.EditorUtility.HorizontalLine(2, Color.gray);
		...
		GUI.color = defColor;
        GUILayout.BeginArea(new Rect(position.width - 85, 60, 100, 65));
        if (GUILayout.Button("Reload", GUILayout.Width(80), GUILayout.Height(40)))
			CheckResources();
		if (GUILayout.Button("Clean Up", GUILayout.Width(80), GUILayout.Height(20)))
		...
		GUILayout.Label("Scene Resources : ");
		GUILayout.Label("");
	...
	void ListTextures()
	{
		...
		for (int i = 0; i < ActiveTextures.Count; i++)
		{
			...
			{
				HalfTextureFormat(tDetails.texture);
				CheckResources();
			}
			if (GUILayout.Button("2x", GUILayout.Width(50)))
			{
				DoubleTextureFormat(tDetails.texture);
				CheckResources();
			}
			GUILayout.EndHorizontal();
		}
	...
	void HalfTextureFormat(Texture texture)
	{
		string path = AssetDatabase.GetAssetPath(texture.GetInstanceID());
		if (path.EndsWith(".asset"))
		{
			Debug.LogWarning("Unspupported texture file type.");
			return;
		}
		...
		if (tips.maxTextureSize == 32)
		{
			return;
		}
		tips.maxTextureSize /= 2;
	...
	void DoubleTextureFormat(Texture texture)
	{
		string path = AssetDatabase.GetAssetPath(texture.GetInstanceID());
		if (path.EndsWith(".asset"))
		{
